...
Status: **first draft** (2026-09-14). Not yet tested with a real connector.
...
1. **Knows their tasks** — they can ask Claude about any Agito task at any time
   ...
   task feed).
2. **Checks in on a schedule the user picks** (every hour by default) on its own, prepares them for new or changed tasks
   by writing to the task feed, and puts work on their Google Calendar.
...
| What agents do | In Agito, **write to the task feed only** — they can't edit, close, or delete tasks (enforced in the database). **Regular tasks:** prepare the user and schedule time. **"Agent: do this" tasks:** work on the task using whatever Cowork allows, and report in the feed. |
...
| Check frequency | **Chosen by the user during onboarding**: every 15 min (advanced), 30 min (advanced), **1 hour (default)**, 2 hours, or 4 hours. Stored as `check_frequency_minutes`. Advanced options note that they use more of the user's Claude plan and may not be available once scheduled tasks run in the cloud. |
...
Decided onboarding options (2026-09-14): every 15 min (advanced), 30 min
(advanced), **1 hour (default)**, 2 hours, 4 hours.
...
| Scheduling window | Asked during Agito onboarding (default **9am–9pm**). Any window works, including overnight (e.g. 10pm–6am) or all day. Agents only place calendar events inside it. Stored in `user_settings`. |
...
**Security:** only the user can mark a task "Agent: do this". Agents can't
create agent tasks or edit tasks, so content the agent reads (files, websites,
...
5. Claude creates the check-in at the user's chosen frequency and runs it once. The user confirms it
   under **Scheduled**, and approves anything it asks for with
...
RULES
- You may use any tools I've allowed in Cowork. For tasks marked "Agent: do this", work on the task itself. For all other tasks, prepare rather than do: apart from creating calendar events, don't take actions on my behalf outside Agito.
...
- Don't close, reopen, or delete tasks, and don't change my tasks. Write to the task feed instead. If you think a task is done, say so in its feed and explain why.
...
- On Google Calendar, only create new events. Never change or delete existing events.

EACH RUN
1. Call get_updates. If there are no updates, stop without writing anything. The response includes my scheduling preferences.
...
4. For all other tasks:
   ...
   - Schedule it unless the feed shows it's already scheduled and the due date and estimate haven't changed since. Use my estimate; if there isn't one, estimate it yourself and say so in the feed.
   - Put it in the next available free slot on my primary Google Calendar that fits the estimate and falls inside my scheduling window, in my calendar's time zone. If the window's end is earlier than its start, it runs overnight; if they're equal, any time is fine. Skip Saturdays and Sundays if my preferences don't allow weekends.
   - If the task has a due date and the next free slot is after it, still book the earliest slot and warn me in the feed.
   - Create the event titled "Agito: <task title>" with the task link from get_task in the description.
5. Write at most one feed entry per task per run, combining your notes and any scheduling ("Scheduled Thu 6:00–7:00pm on Google Calendar."). Keep it short and easy to skim.
...
| `get_preferences` | read-only | Scheduling window, whether weekends are allowed, and check frequency (from onboarding) |
...
| `get_updates` | read-only | Tasks changed since this agent's last acknowledged check, plus tasks that just became due within 24 hours. Excludes the agent's own feed entries. Returns a cursor and the scheduling preferences. |
...
| `create_task` | write | Add a task when the user asks in chat |

Every tool declares read-only / destructive hints. No tools delete, close, or
edit tasks.
...
- **Onboarding** that asks for the scheduling window (default 9am–9pm) and
  whether to schedule on weekends (default yes), and how often Claude checks
  in (default every hour), saved to `user_settings`.
